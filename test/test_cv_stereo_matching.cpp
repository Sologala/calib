...
void splitStereoImage(const cv::Mat &src, cv::Mat &left, cv::Mat &right)
{
    if (src.empty())
        return;
    int width = src.cols;
...
int main()
{
    ...
    while (true)
    {
        cv::Mat img_cv;
        cap >> img_cv;
        splitStereoImage(img_cv, imgL, imgR);
        ...
        // cv::remap(imgR_gray, Right_nice, Right_Stereo_Map1, Right_Stereo_Map2, cv::INTER_LANCZOS4, cv::BORDER_CONSTANT,
        //           0);
